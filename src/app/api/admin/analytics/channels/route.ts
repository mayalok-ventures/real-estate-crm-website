...
import { getAcquisitionChannels } from "@/lib/analytics-store";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getAcquisitionChannels(db, days);

    return NextResponse.json({
      success: true,
      data,
      meta: {
        attributionModel: "Last-Touch with UTM Provenance",
    ...
    console.error("GET /api/admin/analytics/channels error:", error);
    return NextResponse.json(
      { success: false, error: error?.message || "Failed to fetch acquisition channels" },
      { status: 500 }
    );
  }
}
