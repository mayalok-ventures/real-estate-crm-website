...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getTopPages(db, days);

    return NextResponse.json({
      success: true,
      data,
      meta: {
        filter: "Excludes /admin and /api routes",
    ...
    console.error("GET /api/admin/analytics/pages error:", error);
    return NextResponse.json(
      { success: false, error: error?.message || "Failed to fetch top visited pages" },
      { status: 500 }
    );
  }
}
